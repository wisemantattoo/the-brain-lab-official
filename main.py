...
# החלפנו ל-flash כדי למנוע את שגיאת ה-404
...
def get_ai_content():
    ...
    try:
        ...
        print(f"⚠️ שגיאה ב-Gemini: {e}, משתמש בטקסט ברירת מחדל.")
...
def create_video():
    ...
    fps = 25 
    duration = 5 
    print(f"🎬 מרנדר וידאו ב-{fps} FPS עם הטקסט: {text}")
    
    background = ColorClip(size=(1080, 1920), color=(20, 20, 20)).set_duration(duration)
    
    # יצירת טקסט (וודא ש-ImageMagick מותקן כפי שסידרנו ב-Workflow)
...
def upload_to_youtube(file_path, title):
    print("🚀 מתחבר ל-YouTube API...")
    try:
        # טעינת פרטי הלקוח מה-Secret
        ...
        request_body = {
            "snippet": {
                "title": title[:100],
                "description": "#shorts #socialintelligence #ai",
        ...
        print(f"✅ הסרטון עלה בהצלחה! ID: {response.get('id')}")
        
    except Exception as e:
        print(f"❌ שגיאה בהעלאה: {e}")

if __name__ == "__main__":
    if not ALL_SECRETS_EXIST := all([GEMINI_KEY, CLIENT_SECRET_RAW, REFRESH_TOKEN]):
        print("❌ חסרים Secrets בהגדרות ה-GitHub!")
    else:
        video_file, ai_text = create_video()
        upload_to_youtube(video_file, ai_text)
        print("🏁 הבוט סיים את העבודה!")
